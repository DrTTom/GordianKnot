...
/**
 * Unit tests to find unreferenced elements in your class path and own classes. Override the non-final methods for
 * fine-tuning these tests. <br> Maybe junit 5 has more elegant method to feed data into this test where junit 4
 * Parameterized was nur uasable because it requires static methods.
 * <br> Note that Eclipse may resolve the
 * dependencies incorrectly making this test fail! Call from Gradle to be sure.
 *
 * @author TT
 */
public class TestUnreferencedElements
{
    ...
    /**
     * Asserts that there are no unused test classes in your project. Override {@link #getKnownEntryClassNames()} if
     * needed but do not cheat!
     */
    @Test
    ...
    /**
     * Returns an array of fully qualified names of entry classes of your project. You have to specify only those
     * classes which are not automatically recognized as entry classes. <br> Example implementation targets own
     * project.
     */
    ...
    /**
     * Returns an array of fully qualified names of test entry classes of your project. You have to specify only those
     * classes which are not automatically recognized as entry classes. Do not specify classes of your application
     * itself because those should be referenced by some test. In most cases, this method should not be overwritten.
     */
    ...
    /**
     * Returns an array of names or regular expressions for jar files needed by your application but not referenced by
     * any of your classes. Logging runtime jars are a typical example for that. Moreover, gradle may include its worker
     * jar when executing tests, so ignore that one as well.
     */
    ...
    /**
     * Returns the compile class path. Current implementation uses Gradle, override this method if you use a different
     * build system.
     */
    ...
    /**
     * Returns the class path of your application. Current implementation uses Gradle, override this method if you use a
    ...
    private void checkClassUsage(ParsedClassPath cp, String label, String... knownEntryClasses)
    {
        ...
        List<String> unref =
            checker.getUnrefClasses().stream().map(ClassNode::getClassName).collect(Collectors.toList());
    ...
    private void checkLibraryUsage(ParsedClassPath cp, String label, String... knownNeeded)
    {
        ContainerNode root = ProjectCache.getScannedProject(cp, filter);
        ReferenceChecker checker = new ReferenceChecker(root, filter, cp);
        List<String> unref = checker.getUnrefJars().stream().map(Node::getDisplayName).collect(Collectors.toList());
